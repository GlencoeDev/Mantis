...
class RegisterView(generics.GenericAPIView):  # Allows for User Registeration
    ...
    def post(self, request):
        ...
        email_subject = "Welcome to Mantis: Activate you account"

        activation_template = get_template("pages/authentication/activate-email.html")
        activation_render_data = {
        ...
        email = EmailMultiAlternatives(
            email_subject, html_content, "glencoetester@gmail.com", [user_data["email"]]
        )
...
class RequestPasswordResetEmail(
    ...
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            return Response(
                {"Error": "You do not have an account registered"},
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(
                user, token
            ):  # This ensures that user is not re-using the resetlink
                return Response(
...
class SetNewPasswordAPIView(generics.GenericAPIView):
    ...
    def patch(
        self, request
    ):  # We use patch, because we are changing the Users Password
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        return Response(
